...
const serviceRequests = [
  { folio: 'SOL-2026-00142', service: 'Certificaciones y documentos', subtype: 'Certificación municipal', citizen: 'María Pérez · 809-000-0000', municipality: 'Saibot', sector: 'No aplica', department: 'Secretaría Municipal', responsible: 'Ana Secretaría', created: '2026-07-14 10:15 UTC', status: 'Resuelta', priority: 'Normal', nextAction: 'Documento listo para retiro', sla: 'Dentro de plazo · objetivo 24 h · transcurrido 6 h', type: 'Solicitud de servicio', location: 'Retiro en Secretaría', evidence: [], notes: 'Certificaciones no dependen de brigada.', assignedAt: '2026-07-14 10:20 UTC', estimatedCost: 'RD$ 1,500.00', priceStatus: 'Demo', paymentStatus: 'pendiente', futurePaymentMethod: 'No configurado', futurePaymentReference: 'No generada' },
  { folio: 'SOL-2026-00143', service: 'Uso de espacios y servicios municipales', subtype: 'Solicitud de parque o plaza', citizen: 'Junta de vecinos Centro', municipality: 'Saibot', sector: 'Centro', department: 'Servicios Municipales', responsible: 'Luis Operaciones', created: '2026-07-15 09:05 UTC', status: 'En proceso', priority: 'Alta', nextAction: 'Validar disponibilidad del parque', sla: 'Dentro de plazo · objetivo 48 h · transcurrido 3 h', type: 'Solicitud de servicio', location: 'Parque Central', evidence: ['Carta solicitud demo'], notes: 'Actividad comunitaria con fecha/hora capturada por controles nativos.', assignedAt: '2026-07-15 09:12 UTC', estimatedCost: 'RD$ 2,000.00', priceStatus: 'Demo', paymentStatus: 'pendiente', futurePaymentMethod: 'No configurado', futurePaymentReference: 'No generada' },
  { folio: 'SOL-2026-00144', service: 'Limpieza y ornato', subtype: 'Retiro de escombros', citizen: 'Pedro Gómez', municipality: 'Saibot', sector: 'Norte', department: 'Limpieza y Ornato', responsible: 'Limpia Norte', created: '2026-07-12 08:00 UTC', status: 'Vencida', priority: 'Urgente', nextAction: 'Solicitar información adicional y reasignar ruta', sla: 'Fuera de plazo demo · objetivo 24 h · transcurrido 72 h', type: 'Solicitud de servicio', location: 'Calle 4, Norte', evidence: ['Foto demo de escombros'], notes: 'Requiere más datos del ciudadano sin cerrar el caso.', assignedAt: '2026-07-12 08:30 UTC', estimatedCost: 'RD$ 1,500.00', priceStatus: 'Demo', paymentStatus: 'pendiente', futurePaymentMethod: 'No configurado', futurePaymentReference: 'No generada' },
  { folio: 'SOL-2026-00145', service: 'Instalaciones municipales', subtype: 'Instalación municipal', citizen: 'Club Deportivo', municipality: 'Saibot', sector: 'Sur', department: 'Deportes', responsible: 'Carlos Coordinación', created: '2026-07-13 11:40 UTC', status: 'En revisión', priority: 'Baja', nextAction: 'Devolver a revisión si faltan requisitos', sla: 'Dentro de plazo · objetivo 72 h · transcurrido 30 h', type: 'Solicitud de servicio', location: 'Cancha municipal', evidence: [], notes: 'Reapertura controlada disponible después de Resuelta.', assignedAt: '2026-07-13 12:00 UTC', estimatedCost: 'RD$ 3,000.00', priceStatus: 'Demo', paymentStatus: 'pendiente', futurePaymentMethod: 'No configurado', futurePaymentReference: 'No generada' },
];
const serviceStatuses = ['Recibida','Asignada','En proceso','En revisión','Resuelta','Cerrada','Rechazada','Cancelada','Vencida'];
const serviceNotifications = ['solicitud recibida','asignada','en proceso','en revisión','resuelta','documento listo para retiro','cerrada','rechazada','información adicional requerida'];
...
const futureIntegrations = [
  ...
  'Pagos: solo campos futuros; no hay pasarela, cargos ni procesamiento real.',
    ...
    <section class="mp-shell" aria-label="Municipal Panel V2">
      <header class="mp-hero">
        <nav class="mp-nav" aria-label="Navegación Municipal Panel V2">
          ...
          <div class="mp-nav-actions"><a href="#tickets">Incidencias</a><a href="#solicitudes-servicios">Solicitudes de Servicios</a><a href="#todos-casos">Todos los casos</a><a href="#impacto-municipal">Centro de Impacto Municipal</a><a href="#brigadas">Brigadas</a><a href="#mapa">Mapa</a></div>
        </nav>
...
function renderServiceCommandCenter() {
  const kpi = (label, count) => `<article><span>Solicitudes</span><strong>${count}</strong><p>${label}</p></article>`;
  const count = (status) => serviceRequests.filter((item) => item.status === status).length;
  return `<section class="mp-panel service-command" id="solicitudes-servicios" data-service-command-center aria-labelledby="service-command-title">
    <div class="mp-section-head"><div><span class="mp-badge">Centro de comando</span><h2 id="service-command-title">Solicitudes de Servicios</h2><p>Vista interna dentro de Municipal Panel V2; separada de Incidencias y con opción Todos los casos.</p></div><div class="mp-actions"><button>Exportar CSV demo</button><button>Exportar PDF demo</button></div></div>
    <div class="mp-kpis service-kpis">${kpi('solicitudes recibidas', serviceRequests.length)}${kpi('asignadas', count('Asignada'))}${kpi('en proceso', count('En proceso'))}${kpi('en revisión', count('En revisión'))}${kpi('resueltas', count('Resuelta'))}${kpi('cerradas', count('Cerrada'))}${kpi('rechazadas', count('Rechazada'))}${kpi('canceladas', count('Cancelada'))}${kpi('vencidas o fuera de plazo demo', count('Vencida'))}</div>
    <div class="case-switch" id="todos-casos"><strong>Vistas por tipo:</strong><button>Incidencias</button><button>Solicitudes de servicios</button><button>Todos los casos</button><small>No mezclar métricas sin identificar tipo.</small></div>
    <div class="mp-filters service-filters"><select><option>estado</option>${serviceStatuses.map(s=>`<option>${s}</option>`).join('')}</select><select><option>servicio</option></select><select><option>subtipo</option></select><select><option>departamento</option></select><select><option>responsable</option></select><select><option>sector</option></select><select><option>período</option></select><select><option>prioridad</option><option>baja</option><option>normal</option><option>alta</option><option>urgente</option></select></div>
    <div class="mp-table-wrap"><table class="mp-table"><thead><tr><th>Folio SOL-</th><th>Servicio</th><th>Subtipo</th><th>Ciudadano/contacto</th><th>Municipio</th><th>Sector</th><th>Departamento</th><th>Responsable</th><th>Fecha de creación</th><th>Costo estimado</th><th>Estado del precio</th><th>Pago</th><th>Método de pago futuro</th><th>Referencia de pago futura</th><th>Estado</th><th>Prioridad</th><th>Próxima acción</th></tr></thead><tbody>${serviceRequests.map(r=>`<tr><td><strong>${r.folio}</strong></td><td>${r.service}</td><td>${r.subtype}</td><td>${r.citizen}</td><td>${r.municipality}</td><td>${r.sector}</td><td>${r.department}</td><td>${r.responsible}</td><td>${r.created}</td><td>${r.estimatedCost}</td><td>${r.priceStatus}</td><td>${r.paymentStatus}</td><td>${r.futurePaymentMethod}</td><td>${r.futurePaymentReference}</td><td><span class="mp-chip">${r.status}</span></td><td>${r.priority}</td><td>${r.nextAction}</td></tr>`).join('')}</tbody></table></div>
    <article class="mp-panel"><h3>Detalle de solicitud</h3><dl class="mp-detail-list"><div><dt>folio</dt><dd>${serviceRequests[0].folio}</dd></div><div><dt>historial</dt><dd>Recibida → Asignada → En proceso → En revisión → Resuelta</dd></div><div><dt>datos del solicitante</dt><dd>${serviceRequests[0].citizen}</dd></div><div><dt>ubicación</dt><dd>${serviceRequests[0].location}</dd></div><div><dt>evidencia</dt><dd>No requerida</dd></div><div><dt>campos específicos del servicio</dt><dd>Certificación municipal · retiro administrativo</dd></div><div><dt>departamento</dt><dd>${serviceRequests[0].department}</dd></div><div><dt>responsable</dt><dd>${serviceRequests[0].responsible} · asignado ${serviceRequests[0].assignedAt}</dd></div><div><dt>costo estimado</dt><dd>${serviceRequests[0].estimatedCost}</dd></div><div><dt>estado del precio</dt><dd>${serviceRequests[0].priceStatus}</dd></div><div><dt>pago</dt><dd>${serviceRequests[0].paymentStatus}</dd></div><div><dt>método de pago futuro</dt><dd>${serviceRequests[0].futurePaymentMethod}</dd></div><div><dt>referencia de pago futura</dt><dd>${serviceRequests[0].futurePaymentReference}</dd></div><div><dt>estado</dt><dd>${serviceRequests[0].status}</dd></div><div><dt>notas</dt><dd>${serviceRequests[0].notes}</dd></div><div><dt>SLA visual demo</dt><dd>${serviceRequests[0].sla}</dd></div><div><dt>timeline</dt><dd>Solicitud recibida, asignada, resuelta, lista para retiro.</dd></div></dl><h3>Acciones visuales demo</h3><div class="mp-actions">${['asignar','marcar en proceso','enviar a revisión','resolver','cerrar','rechazar','cancelar','solicitar información adicional','devolver a revisión','reabrir controladamente'].map(a=>`<button>${a}</button>`).join('')}</div><h3>Notificaciones preparadas</h3><ul>${serviceNotifications.map(n=>`<li>${n}</li>`).join('')}</ul></article>
  </section>`;
}
