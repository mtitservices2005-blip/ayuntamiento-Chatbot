...
function renderServiceCommandCenter() {
  ...
  return `<section class="mp-panel service-command" id="solicitudes-servicios" data-service-command-center aria-labelledby="service-command-title">
    ...
    <div class="mp-table-wrap"><table class="mp-table"><thead><tr><th>Folio SOL-</th><th>Servicio</th><th>Subtipo</th><th>Ciudadano/contacto</th><th>Municipio</th><th>Sector</th><th>Departamento</th><th>Responsable</th><th>Fecha de creación</th><th>Estado</th><th>Prioridad</th><th>Próxima acción</th></tr></thead><tbody>${serviceRequests.map(r=>`<tr><td><strong>${r.folio}</strong></td><td>${r.service}</td><td>${r.subtype}</td><td>${r.citizen}</td><td>${r.municipality}</td><td>${r.sector}</td><td>${r.department}</td><td>${r.responsible}</td><td>${r.created}</td><td><span class="mp-chip">${r.status}</span></td><td>${r.priority}</td><td>${r.nextAction}</td></tr>`).join('')}</tbody></table></div>
    <article class="mp-panel"><h3>Detalle de solicitud</h3><dl class="mp-detail-list"><div><dt>folio</dt><dd>${serviceRequests[0].folio}</dd></div><div><dt>historial</dt><dd>Recibida → Asignada → En proceso → En revisión → Resuelta</dd></div><div><dt>datos del solicitante</dt><dd>${serviceRequests[0].citizen}</dd></div><div><dt>ubicación</dt><dd>${serviceRequests[0].location}</dd></div><div><dt>evidencia</dt><dd>No requerida</dd></div><div><dt>campos específicos del servicio</dt><dd>Certificación municipal · retiro administrativo</dd></div><div><dt>departamento</dt><dd>${serviceRequests[0].department}</dd></div><div><dt>responsable</dt><dd>${serviceRequests[0].responsible} · asignado ${serviceRequests[0].assignedAt}</dd></div><div><dt>estado</dt><dd>${serviceRequests[0].status}</dd></div><div><dt>notas</dt><dd>${serviceRequests[0].notes}</dd></div><div><dt>SLA visual demo</dt><dd>${serviceRequests[0].sla}</dd></div><div><dt>timeline</dt><dd>Solicitud recibida, asignada, resuelta, lista para retiro.</dd></div></dl><h3>Acciones visuales demo</h3><div class="mp-actions">${['asignar','marcar en proceso','enviar a revisión','resolver','cerrar','rechazar','cancelar','solicitar información adicional','devolver a revisión','reabrir controladamente'].map(a=>`<button>${a}</button>`).join('')}</div><h3>Notificaciones preparadas</h3><ul>${serviceNotifications.map(n=>`<li>${n}</li>`).join('')}</ul></article>
  </section>`;
}
